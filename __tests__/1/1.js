...
const exercise21 = (person, [minAge, maxAge]) => {
  ...
  for(let item of person.subordinates){
    if(item.age>=minAge && item.age<=maxAge) len++
  }
  return len
};
...
const exercise51 = (color) => {
  let totalPeople = 0;
  const getPeopleEyeColor = (person) => {
    if (person.eyeColor === color) 
    {
      totalPeople++;
    }
    person.subordinates.forEach((subordinate) => {
      getPeopleEyeColor(subordinate);
    });
  };
  getPeopleEyeColor(CruzHarrell);
  return totalPeople;
};
...
const exercise53 = () => {
  ...
  const getPersons = (person) => {
    person.subordinates.forEach((subordinate) => {
      let personCompany = exercise11(person.email);
      let managerCompany = exercise11(subordinate.email);
      if (personCompany === managerCompany)
      {
        person.push(subordinate.name.split(' ')[0]);
        getPersons(subordinate);
      }
    });
  };

  getPersons(CruzHarrell);
  return persons;
};
